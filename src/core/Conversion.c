...
// ----------------------------------------------------------------------
// Range Conversion
static int RangeConversionElemwiseLeq(const void * first, const void * second, ChannelType * type) {
    switch (type->con) {
        case CHANNEL_DOUBLE:
...
static int RangeConversionElemwiseGeq(const void * first, const void * second, ChannelType * type) {
    switch (type->con) {
        case CHANNEL_DOUBLE:
            return *(double *) first >= *(double *) second;
        case CHANNEL_INTEGER:
...
static int LinearConversionIsEmpty(LinearConversion * conversion) {
    ...
    case CHANNEL_INTEGER:
        return (!conversion->factor || *(int64_t *) ChannelValueDataPointer(conversion->factor) == 1) &&
               (!conversion->offset || *(int64_t *) ChannelValueDataPointer(conversion->offset) == 0);
    case CHANNEL_ARRAY:
...
static McxStatus CheckTypesValidForConversion(const ChannelType * destType,
                                              const ChannelType * expectedDestType)
{
...
static McxStatus TypeConversionConvertIntDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeDouble)) {
        return RETURN_ERROR;
    }

    dest->ref.value->value.d = (double) *((int64_t *) src);

    return RETURN_OK;
}

static McxStatus TypeConversionConvertDoubleInt(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeInteger)) {
        return RETURN_ERROR;
    }

    dest->ref.value->value.i = (int64_t) floor(*((double *) src) + 0.5);

    return RETURN_OK;
}

static McxStatus TypeConversionConvertBoolDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeDouble)) {
        return RETURN_ERROR;
    }

    dest->ref.value->value.d = *((int64_t *) src) != 0 ? 1. : 0.;

    return RETURN_OK;
}

static McxStatus TypeConversionConvertDoubleBool(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
...
static McxStatus TypeConversionConvertBoolInteger(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeInteger)) {
        return RETURN_ERROR;
    }

    dest->ref.value->value.i = *((int64_t *) src) != 0 ? 1 : 0;

    return RETURN_OK;
}

static McxStatus TypeConversionConvertIntegerBool(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeBool)) {
        return RETURN_ERROR;
    }
...
static McxStatus TypeConversionConvertArrayDoubleToDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeDouble)) {
        return RETURN_ERROR;
    }
...
static McxStatus TypeConversionConvertArrayIntegerToDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeDouble)) {
        return RETURN_ERROR;
    }

    if (conversion->sourceSlice) {
        dest->ref.value->value.d = (double) *((int64_t *) ((mcx_array *) src)->data + conversion->sourceSlice->startIdxs[0]);
    } else {
        dest->ref.value->value.d = (double) *(int64_t *) ((mcx_array *) src)->data;
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertArrayBoolToDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeDouble)) {
        return RETURN_ERROR;
    }

    if (conversion->sourceSlice) {
        dest->ref.value->value.d = *((int64_t *) ((mcx_array *) src)->data + conversion->sourceSlice->startIdxs[0]) != 0 ? 1. : 0.;
    } else {
        dest->ref.value->value.d = *(int64_t *) ((mcx_array *) src)->data != 0 ? 1. : 0.;
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertArrayDoubleToInteger(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeInteger)) {
        return RETURN_ERROR;
    }

    if (conversion->sourceSlice) {
        dest->ref.value->value.i = (int64_t) floor(*((double *) ((mcx_array *) src)->data + conversion->sourceSlice->startIdxs[0]) + 0.5);
    } else {
        dest->ref.value->value.i = (int64_t) floor(*(double *) ((mcx_array *) src)->data + 0.5);
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertArrayIntegerToInteger(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeInteger)) {
        return RETURN_ERROR;
    }

    if (conversion->sourceSlice) {
        dest->ref.value->value.i = *((int64_t *) ((mcx_array *) src)->data + conversion->sourceSlice->startIdxs[0]);
    } else {
        dest->ref.value->value.i = *(int64_t *) ((mcx_array *) src)->data;
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertArrayBoolToInteger(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeInteger)) {
        return RETURN_ERROR;
    }

    if (conversion->sourceSlice) {
        dest->ref.value->value.i = *((int64_t *) ((mcx_array *) src)->data + conversion->sourceSlice->startIdxs[0]) != 0 ? 1 : 0;
    } else {
        dest->ref.value->value.i = *(int64_t *) ((mcx_array *) src)->data != 0 ? 1 : 0;
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertArrayDoubleToBool(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeBool)) {
        return RETURN_ERROR;
    }
...
static McxStatus TypeConversionConvertArrayIntegerToBool(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeBool)) {
        return RETURN_ERROR;
    }
...
static McxStatus TypeConversionConvertArrayBoolToBool(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckTypesValidForConversion(ChannelValueReferenceGetType(dest), &ChannelTypeBool)) {
        return RETURN_ERROR;
    }
...
static McxStatus TypeConversionConvertDoubleToArrayDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
...
static McxStatus TypeConversionConvertIntegerToArrayDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckArrayReferencingOnlyOneElement(dest, &ChannelTypeDouble)) {
        return RETURN_ERROR;
    }

    if (dest->type == CHANNEL_VALUE_REF_VALUE) {
        *(double *) dest->ref.value->value.a.data = (double) *((int64_t *) src);
    } else {
        double * data = dest->ref.slice.ref->value.a.data;
        *(data + dest->ref.slice.dimension->startIdxs[0]) = (double) *((int64_t *) src);
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertBoolToArrayDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckArrayReferencingOnlyOneElement(dest, &ChannelTypeDouble)) {
        return RETURN_ERROR;
    }

    if (dest->type == CHANNEL_VALUE_REF_VALUE) {
        *(double *) dest->ref.value->value.a.data = *((int64_t *) src) != 0 ? 1. : 0.;
    } else {
        double * data = dest->ref.slice.ref->value.a.data;
        *(data + dest->ref.slice.dimension->startIdxs[0]) = *((int64_t *) src) != 0 ? 1. : 0.;
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertDoubleToArrayInteger(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckArrayReferencingOnlyOneElement(dest, &ChannelTypeInteger)) {
        return RETURN_ERROR;
    }

    if (dest->type == CHANNEL_VALUE_REF_VALUE) {
        *(int64_t *) dest->ref.value->value.a.data = (int64_t) floor(*((double *) src) + 0.5);
    } else {
        int64_t * data = dest->ref.slice.ref->value.a.data;
        *(data + dest->ref.slice.dimension->startIdxs[0]) = (int64_t) floor(*((double *) src) + 0.5);
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertIntegerToArrayInteger(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckArrayReferencingOnlyOneElement(dest, &ChannelTypeInteger)) {
        return RETURN_ERROR;
    }

    if (dest->type == CHANNEL_VALUE_REF_VALUE) {
        *(int64_t *) dest->ref.value->value.a.data = *(int64_t *) src;
    } else {
        int64_t * data = dest->ref.slice.ref->value.a.data;
        *(data + dest->ref.slice.dimension->startIdxs[0]) = *(int64_t *) src;
    }

    return RETURN_OK;
}

static McxStatus TypeConversionConvertBoolToArrayInteger(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckArrayReferencingOnlyOneElement(dest, &ChannelTypeInteger)) {
        return RETURN_ERROR;
    }

    if (dest->type == CHANNEL_VALUE_REF_VALUE) {
        *(int64_t *) dest->ref.value->value.a.data = *((int64_t *) src) != 0 ? 1 : 0;
    } else {
        int64_t * data = dest->ref.slice.ref->value.a.data;
        *(data + dest->ref.slice.dimension->startIdxs[0]) = *((int64_t *) src) != 0 ? 1 : 0;
    }
...
static McxStatus TypeConversionConvertDoubleToArrayBool(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckArrayReferencingOnlyOneElement(dest, &ChannelTypeBool)) {
        return RETURN_ERROR;
    }

    if (dest->type == CHANNEL_VALUE_REF_VALUE) {
        *(int64_t *) dest->ref.value->value.a.data = *((double *) src) > 0 ? 1 : 0;
    } else {
        int64_t * data = dest->ref.slice.ref->value.a.data;
...
static McxStatus TypeConversionConvertIntegerToArrayBool(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckArrayReferencingOnlyOneElement(dest, &ChannelTypeBool)) {
        return RETURN_ERROR;
    }
...
static McxStatus TypeConversionConvertBoolToArrayBool(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
    if (RETURN_ERROR == CheckArrayReferencingOnlyOneElement(dest, &ChannelTypeBool)) {
        return RETURN_ERROR;
    }
...
static size_t IndexOfElemInSrcArray(size_t dest_idx, const mcx_array * src_array, const ChannelDimension * src_dim, const ChannelValueReference * dest) {
    if (src_dim) {
        if (dest->type == CHANNEL_VALUE_REF_VALUE) {
...
typedef struct Array2ArrayCtx {
    const mcx_array * src_array;
    const ChannelDimension * src_dim;
    ChannelValueReference * dest;
} Array2ArrayCtx;

static McxStatus IntegerArrayToDouble(void * element, size_t idx, ChannelType * type, const void * ctx) {
    const Array2ArrayCtx * context = (const Array2ArrayCtx *) ctx;
    ...
    *(double *) element = (double) *((int64_t *) src_elem);

    return RETURN_OK;
}

static McxStatus BoolArrayToDouble(void * element, size_t idx, ChannelType * type, const void * ctx) {
    const Array2ArrayCtx * context = (const Array2ArrayCtx *) ctx;
    size_t i = IndexOfElemInSrcArray(idx, context->src_array, context->src_dim, context->dest);
    void * src_elem = mcx_array_get_elem_reference(context->src_array, i);
    ...
    *(double *) element = *((int64_t *) src_elem) != 0 ? 1. : 0.;

    return RETURN_OK;
}

static McxStatus BoolArrayToInteger(void * element, size_t idx, ChannelType * type, const void * ctx) {
    const Array2ArrayCtx * context = (const Array2ArrayCtx *) ctx;
    size_t i = IndexOfElemInSrcArray(idx, context->src_array, context->src_dim, context->dest);
    void * src_elem = mcx_array_get_elem_reference(context->src_array, i);
    ...
    *(int64_t *) element = *((int64_t *) src_elem) != 0 ? 1 : 0;

    return RETURN_OK;
}

static McxStatus DoubleArrayToInteger(void * element, size_t idx, ChannelType * type, const void * ctx) {
    const Array2ArrayCtx * context = (const Array2ArrayCtx *) ctx;
    size_t i = IndexOfElemInSrcArray(idx, context->src_array, context->src_dim, context->dest);
    void * src_elem = mcx_array_get_elem_reference(context->src_array, i);
    ...
    *(int64_t *) element = (int64_t) floor(*((double *) src_elem) + 0.5);

    return RETURN_OK;
}

static McxStatus DoubleArrayToBool(void * element, size_t idx, ChannelType * type, const void * ctx) {
    const Array2ArrayCtx * context = (const Array2ArrayCtx *) ctx;
    size_t i = IndexOfElemInSrcArray(idx, context->src_array, context->src_dim, context->dest);
    void * src_elem = mcx_array_get_elem_reference(context->src_array, i);
    ...
    *(int64_t *) element = *((double *) src_elem) > 0 ? 1 : 0;

    return RETURN_OK;
}

static McxStatus IntegerArrayToBool(void * element, size_t idx, ChannelType * type, const void * ctx) {
    const Array2ArrayCtx * context = (const Array2ArrayCtx *) ctx;
    size_t i = IndexOfElemInSrcArray(idx, context->src_array, context->src_dim, context->dest);
    void * src_elem = mcx_array_get_elem_reference(context->src_array, i);
...
static McxStatus TypeConversionConvertArrayIntegerToArrayDouble(TypeConversion * conversion, ChannelValueReference * dest, const void * src) {
